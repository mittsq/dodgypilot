...
class CarState(CarStateBase):
  ...
  def update(self, cp, cp_cam):
    ...
    if ret.gasPressed and not self.gas_pressed_prev:
      self.needs_angle_offset_zss = True # reset zss tolerance on gas press
      self.out_of_tolerance_counter = 0
    self.gas_pressed_prev = ret.gasPressed # only count one frame so ZSS is not reset continuously
